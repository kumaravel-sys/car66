// main.js — Three.js starter with simple vehicle physics approximations
...
// ---------- Ground / heightmap ----------
const size = 2048; // world tile size (one tile for starter)
const segments = 256;
const geometry = new THREE.PlaneGeometry(size, size, segments, segments);
geometry.rotateX(-Math.PI / 2);

// Simple procedural height: you can load actual heightmap image and displace
for (let i = 0; i < geometry.attributes.position.count; i++) {
  const y = (Math.sin(geometry.attributes.position.getX(i) * 0.01) +
             Math.cos(geometry.attributes.position.getZ(i) * 0.01 * 0.7)) * 8;
...
const vehicle = {
  ...
  wheelBase: 2.7,
  maxSteer: Math.PI / 6,
  dragCoef: 0.4257, // aerodynamic factor (tweak for realism)
  rollingResistance: 12,
  wheelInfos: [] // per-wheel suspension & contact info
};

// create a placeholder visual car (if you have a GLTF, it will be replaced)
...
// load a glTF car if you have one in assets/car.glb
...
// ---------- Camera modes ----------
let chaseCam = true;
function toggleCamera() { chaseCam = !chaseCam; if (!chaseCam) controls.target.copy(vehicle.position); }
...
function physicsStep() {
  ...
  const maxEngine = 8000; // N top drive force
  ...
  // yaw rotation from steering (very simplified bicycle model)
  const angularVel = (vehicle.velocity.z !== 0 ? (vehicle.velocity.length() / vehicle.wheelBase) * Math.tan(vehicle.steerAngle) : 0);
  // apply yaw by rotating quaternion a bit around Y based on lateral velocity and steer
  ...
  // apply velocity in local forward direction (approx)
